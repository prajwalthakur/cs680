...
class Config():
    ...
    SAVE_DIR = os.path.join(os.getcwd() , 'swin_pre_fine_tuned/')
    train_df = pd.read_csv(BASE_DIR  +  '/train.csv')
    TRAIN_VAL_SPLIT_SIZE = 0.16
    TRAIN_BATCH_SIZE = 64
    VAL_BATCH_SIZE = 64
    TEST_BATCH_SIZE = 20
    LR_MAX = 1e-4 
...
if CONFIG.WANDB_INIT:
    wandb.login()
    wandb.init(project="cs680v3",group="dino_V2",name="swin_transformer_pre_fine_tuned",
            config = {
...
class ImageBackbone_dinoV2(nn.Module):
    def __init__(self, backbone_name, weight_path, out_features, fixed_feature_extractor=False):
        ...
        self.backbone.load_state_dict(torch.load('/home/prajwal/cs680/cs680_kaggle/backup1/model_08.pth'))
...
class BestModelSaveCallback:
    ...
    def __call__(self, accuracy,model):
        if accuracy > self.best_accuracy:
            ...
            submission_df[["id"]  + CONFIG.TRAITS_NAME ].to_csv('submission_swin_pre_fine_tuned.csv', index=False)
            print("Submit!")
...
#model.load_state_dict(torch.load('/home/prajwal/cs680/cs680_kaggle/data/submission_dinoV2_not_fine_tuned_prev.pth'))
model.to(DEVICE)      
...
MODEL_NAME_SAVE = 'submission_swin_pre_fine_tuned.pth'
best_model_callback = BestModelSaveCallback(save_path=os.path.join(CONFIG.SAVE_DIR,MODEL_NAME_SAVE))
